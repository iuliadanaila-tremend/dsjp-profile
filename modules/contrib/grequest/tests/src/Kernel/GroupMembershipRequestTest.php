...
class GroupMembershipRequestTest extends GroupKernelTestBase {
  ...
  public function testRequestWithMemberAsUser() {
    ...
    $this->assertTrue(in_array('User "%name" is already a member of group', $messages), 'The validation of group membership request is found.');
  }
  ...
  public function testRequestStatusUpdate() {
    ...
    $this->assertEquals($group_membership_request->grequest_status->value, GroupMembershipRequest::REQUEST_PENDING);

    $this->membershipRequestManager->approve($group_membership_request);
    $this->assertEquals($group_membership_request->grequest_status->value, GroupMembershipRequest::REQUEST_APPROVED);

    $this->membershipRequestManager->reject($group_membership_request);
    $this->assertEquals($group_membership_request->grequest_status->value, GroupMembershipRequest::REQUEST_REJECTED);
  }
