 ...
 */
final class NegotiationLanguageSelectionPageForm extends ConfigFormBase implements ContainerInjectionInterface {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($form_state->get(['conditions']) as $condition) {
      ...
      if (\array_key_exists($condition->getPluginId(), $condition->getConfiguration())) {
        $this
          ->config
          ->set(
            $condition->getPluginId(),
            $condition->getConfiguration()[$condition->getPluginId()]
          );
      }
    }
