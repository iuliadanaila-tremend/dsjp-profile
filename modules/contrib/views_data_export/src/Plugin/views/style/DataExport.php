...
class DataExport extends Serializer {
  ...
  public function defineOptions() {
    ...
    $options['csv_settings']['contains'] = [
      ...
      'hide_empty_columns' => ['default' => FALSE],
      'trim' => ['default' => TRUE],
        ...
        if (in_array('csv', $format_options)) {
          ...
          $form['csv_settings'] = [
            ...
            'hide_empty_columns' => [
              ...
              '#description' => $this->t('Hide empty columns from CSV if no one row has value for that.'),
              '#default_value' => $csv_options['hide_empty_columns'],
            ],
            'encoding' => [
              '#type' => 'radios',
  ...
  public function render() {
    ...
    if ($format === 'csv' && !empty($this->options['csv_settings']['hide_empty_columns'])) {
      // Prepare list of fields, which has value for at least one row.
      $non_empty_columns = [];
      foreach ($rows as $row) {
        foreach ($row as $field => $value) {
          if (empty($non_empty_columns[$field]) && !empty($value)) {
            $non_empty_columns[$field] = TRUE;
          }
        }
      }

      // Remove columns, which has no values.
      foreach ($rows as &$row) {
        foreach ($row as $field => $value) {
          if (empty($non_empty_columns[$field])) {
            unset($row[$field]);
          }
        }
      }
